# Cloud-Init DataSource for QEMU
#
...
This module provides a cloud-init datasource for QEMU systems and supports
...
class DataSourceQEMU(sources.DataSource):
    """
    ...
    def __init__(self, sys_cfg, distro, paths, ud_proc=None):
        ...
        # 3. A boolean tells whether the transport requires QEMU platform
    ...
    def _get_data(self):
        ...
        transports, the order should match the order in the dscheck_QEMU
        ...
        for (
            data_access_method,
            get_data_fn,
            require_qemu_platform,
        ) in self.possible_data_access_method_list:
            if require_qemu_platform and not is_qemu_platform():
                continue
    ...
    def get_guestinfo_data_fn(self):
        ...
        def query_guestinfo(rpctool, rpctool_fn):
            md, ud, vd, nd = None, None, None, None
...
#            nd = guestinfo("networkdata", rpctool, rpctool_fn)
...
def is_qemu_platform():
    ...
    if system_type is None:
        ...
        return False
    elif "qemu" not in system_type.lower():
        LOG.debug("Not a QEMU platform")
        return False
    return True
...
def get_guestproperty_key_name(key):
    return "/QEMU/Guest-Property/" + key


def get_guestinfo_envvar_key_name(key):
    return ("QEMU" + get_guestproperty_key_name(key)).upper().replace("/", "_", -1)
...
def exec_vboxcontrol(tool, prop, arg):
    (stdout, stderr) = subp([tool, prop, arg])
...
datasources = [
    (DataSourceQEMU, (sources.DEP_FILESYSTEM,)),  # Run at init-local
    (DataSourceQEMU, (sources.DEP_FILESYSTEM, sources.DEP_NETWORK)),
]
...
def main():
    ...
    pic = atomic_helper.json_dumps(metadata)
    print(pic)
